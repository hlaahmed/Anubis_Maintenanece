...
class text_widget(QWidget):
    ...
    def itUI(self):
        ...
        text = QTextEdit()
        cstext = QTextEdit()
        # Python_Coloring.CsHighlighter(cstext)
        Python_Coloring.Highlighter(text)
        hbox = QHBoxLayout()
        # if(extension.toPlainText()== 'py'):
        hbox.addWidget(text)
        # else:
        #     hbox.addWidget(cstext)
        self.setLayout(hbox)
...
class Widget(QWidget):
    ...
    def on_clicked(self, index):
        ...
        if nn[0]:
            ...
            print(extension.toPlainText())
            f = open(nn[0],'r')
            with f:
                data = f.read()
                if(extension.toPlainText()=='py' or extension.toPlainText()=='cs'):
                    Python_Coloring.setExtension(extension.toPlainText())
                    text.setText(data)
                # else:
                #     cstext.setText(data)
                # self.initUI()
